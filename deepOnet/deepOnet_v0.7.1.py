...
- Exports example temperature and humidity maps using IDW interpolation.

Important scope note
--------------------
This version does NOT compute thermal comfort. It only generates temperature
and humidity field maps.
"""
...
PARAMS: Dict[str, Any] = {
    "seed": 20,
    "device": "auto",
    "experiment_name": "lab1_to_lab2_transfer_v0.7.1",
    "data_root": "./data",
    "resolution": "5min",
    ...
    "subset_search_active_counts": [5, 3, 2],
    "subset_search_generation_mode": {5: "sampled", 3: "exhaustive", 2: "exhaustive"},
    "subset_search_sample_limit": {5: 20, 3: 150, 2: None},
    ...
    "lab1_eval_metrics_name": "thermodt_lab1_reference_metrics_v071.csv",
    "manual_lab2_metrics_name": "thermodt_lab2_manual_metrics_v071.csv",
    "manual_lab2_sensor_summary_name": "thermodt_lab2_manual_sensor_summary_v071.csv",
    "manual_lab2_protocol_summary_name": "thermodt_lab2_manual_protocol_summary_v071.csv",
    "subset_search_metrics_name": "thermodt_subset_search_metrics_v071.csv",
    "subset_search_sensor_summary_name": "thermodt_subset_search_sensor_summary_v071.csv",
    "subset_search_protocol_summary_name": "thermodt_subset_search_protocol_summary_v071.csv",
    "subset_search_best_subsets_name": "thermodt_subset_search_best_subsets_v071.csv",
    "selected_protocols_name": "thermodt_selected_protocols_v071.csv",
    "unified_metrics_name": "thermodt_unified_metrics_v071.csv",
    "unified_sensor_summary_name": "thermodt_unified_sensor_summary_v071.csv",
    "unified_protocol_summary_name": "thermodt_unified_protocol_summary_v071.csv",
    "benchmark_regime_summary_name": "thermodt_benchmark_regime_summary_v071.csv",
    "benchmark_regime_pivot_r2_name": "thermodt_benchmark_regime_pivot_r2_v071.csv",
    "training_runs_summary_name": "thermodt_training_runs_summary_v071.csv",
    "summary_json_name": "thermodt_summary_v071.json",
    "summary_md_name": "thermodt_summary_v071.md",
    "overview_figure_name": "transfer_overview_topk_v071.png",
    "sensor_map_temperature_name": "transfer_sensor_map_temperature_v071.png",
    "sensor_map_humidity_name": "transfer_sensor_map_humidity_v071.png",
    "subset_compare_k5_name": "transfer_subset_compare_k5_v071.png",
    "subset_compare_k3_name": "transfer_subset_compare_k3_v071.png",
    "subset_compare_k2_name": "transfer_subset_compare_k2_v071.png",
    "field_map_temperature_name": "field_map_temperature_example_v071.png",
    "field_map_humidity_name": "field_map_humidity_example_v071.png",
}
...
def train_model(cfg: Mapping[str, Any], normalized: NormalizedLabData, split_masks: SplitMasks, train_key: str, val_key: str, device: torch.device, out_dir: Path, best_name: str, final_name: str, lr: float, epochs: int, patience: int, min_delta: float, seed: int, init_state_dict: Mapping[str, Any] | None = None, tag: str = "train", transfer_train_mode: str | None = None) -> Tuple[Path, Path, Dict[str, Any]]:
    ...
    print("=" * 96)
    print(f"DeepONet v0.7.1 | {tag} | lab={normalized.lab_name} | device={device}")
    print("=" * 96)
...
def build_generated_protocols(cfg: Mapping[str, Any], sensor_ids: Sequence[int]) -> List[TransferProtocol]:
    ...
    protocols: List[TransferProtocol] = []
    for k in [int(x) for x in cfg["subset_search_active_counts"]]:
...
def _mode_label(mode: str) -> str:
    return {
        ...
        "ground_truth": "LAB2 ground truth",
...
def _plot_example_field_map(normalized: NormalizedLabData, eval_mask: np.ndarray, selected_df: pd.DataFrame, models_by_mode: Mapping[str, nn.Module], device: torch.device, cfg: Mapping[str, Any], variable: str, outfile: Path) -> Dict[str, Any]:
    ...
    row = selected_df[(selected_df["active_count"] == int(cfg["field_map_active_count"])) & (selected_df["rank_within_k"] == int(cfg["field_map_rank_within_k"]))].iloc[0]
    ...
    grid_nx = int(cfg["field_map_grid_nx"])
    grid_ny = int(cfg["field_map_grid_ny"])
    power = float(cfg["field_map_idw_power"])
    panels: List[Tuple[str, Dict[int, float], str]] = []
    panels.append(("ground_truth", {sid: float(actual[i, var_idx]) for i, sid in enumerate(normalized.sensor_ids)}, "Lab2 ground truth (all 13)"))
    if bool(cfg.get("field_map_include_sparse_idw_baseline", True)):
        panels.append(("sparse_idw_baseline", {sid: float(actual[normalized.sensor_ids.index(sid), var_idx]) for sid in active_sensors}, f"Sparse IDW baseline ({len(active_sensors)} active)"))
    for mode, model in models_by_mode.items():
        pred_grid = predict_sensor_grid_for_time(model, normalized, global_idx, active_sensors, device, cfg)
        panels.append((mode, {sid: float(pred_grid[i, var_idx]) for i, sid in enumerate(normalized.sensor_ids)}, _mode_label(mode)))
    vmin, vmax = float("inf"), float("-inf")
    surfaces = []
    for mode, values_by_sensor, panel_title in panels:
        xx, yy, zz = _idw_surface(coords, values_by_sensor, room_w, room_h, grid_nx, grid_ny, power)
        surfaces.append((mode, values_by_sensor, panel_title, xx, yy, zz))
        vmin = min(vmin, float(np.nanmin(zz)))
        vmax = max(vmax, float(np.nanmax(zz)))
    ncols = len(surfaces)
    ...
    im = None
    for ax, (mode, values_by_sensor, panel_title, _, _, zz) in zip(axes, surfaces):
        im = ax.imshow(zz, origin="lower", extent=(0, room_w, 0, room_h), cmap=cmap, vmin=vmin, vmax=vmax, aspect="auto", interpolation="bilinear", zorder=0)
        ...
        ax.grid(True, alpha=0.15)
        for sid, (x, y) in coords.items():
            marker_face = "white" if mode == "ground_truth" else ("black" if sid in active_sensors else "none")
            ax.scatter([x], [y], s=110, facecolor=marker_face, edgecolor="black", linewidth=1.0, zorder=3)
            if sid in values_by_sensor:
                val = float(values_by_sensor[sid])
                ax.text(x + 0.02 * room_w, y + 0.02 * room_h, f"{sid}\n{val:.2f}", fontsize=8.5, color="black", zorder=4, bbox=dict(boxstyle="round,pad=0.16", facecolor=(1, 1, 1, 0.35), edgecolor="none"))
    if im is not None:
        cb = fig.colorbar(im, cax=cax)
        ...
        cb.ax.tick_params(labelsize=10)
    legend_handles = [plt.Line2D([0], [0], marker="o", linestyle="None", color="black", markerfacecolor="black", markersize=8, label="Active sensor"), plt.Line2D([0], [0], marker="o", linestyle="None", color="black", markerfacecolor="none", markersize=8, label="Inactive sensor")]
    ...
    fig.suptitle(f"Lab2 {variable.capitalize()} field map example — protocol {_pretty_subset_label(active_sensors)} — time {ts}", fontsize=18, fontweight="bold", y=0.98)
    fig.subplots_adjust(left=0.05, right=0.965, top=0.87, bottom=0.16, wspace=0.18)
    fig.savefig(outfile, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return {"protocol_name": str(row["protocol_name"]), "active_sensors": list(active_sensors), "active_count": int(len(active_sensors)), "time_global_index": int(global_idx), "time_eval_local_index": int(local_idx), "timestamp": ts, "variable": variable, "outfile": str(outfile)}
...
def _build_summary_markdown(selected_df: pd.DataFrame, regime_summary: pd.DataFrame, training_summary: pd.DataFrame, output_path: Path, cfg: Mapping[str, Any]) -> None:
    lines: List[str] = []
    lines.append("# Unified transfer benchmark (v0.7.1)")
    lines.append("")
    lines.append("This summary was generated by one single v0.7.1 pipeline.")
    lines.append("")
...
def main(cfg: Dict[str, Any] | None = None) -> None:
    ...
    print("=" * 100)
    print("DeepONet v0.7.1 | unified comparable transfer benchmark")
    print("=" * 100)
    ...
    if bool(cfg.get("run_subset_search", True)):
        ...
        metrics_df, sensor_df, protocol_df = evaluate_active_protocols(available_models[source_mode], lab2_norm, lab2_split.masks["test"], subset_protocols, device, source_mode, cfg)
        best_df = protocol_df.pivot_table(index=["transfer_mode", "protocol_name", "display_name", "active_total_nominal", "masked_total"], columns="variable", values="r2").reset_index()
        for col in ["temperature", "humidity"]:
            if col not in best_df.columns:
                best_df[col] = np.nan
        best_df = best_df.rename(columns={"active_total_nominal": "active_count", "temperature": "r2_temperature", "humidity": "r2_humidity"})
    ...
    if bool(cfg.get("run_unified_benchmark", True)):
        ...
        if 5 in [int(x) for x in cfg["selected_active_counts"]]:
        ...
        if 3 in [int(x) for x in cfg["selected_active_counts"]]:
        ...
        if 2 in [int(x) for x in cfg["selected_active_counts"]]:
