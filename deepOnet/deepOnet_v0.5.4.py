...
"""
DeepONet v0.5.4 — compact comparison suite with LSTM branch
====================================================================
...
PARAMS: Dict[str, Any] = {
    ...
    "experiment_name": "lab1_temp_humidity_v0.5.4_lstm6",
    "data_root": "./data",
    "lab": "lab1",
    "resolution": "5min",
    ...
    "latent_dim": 128,
    "depth": 4,
    "branch_mode": "lstm",  # "snapshot" or "lstm"
    "sequence_length": 6,
    "lstm_hidden": 64,
    "lstm_layers": 1,
    "lstm_dropout": 0.0,
    ...
    "weight_decay": 0.0,
    "num_workers": 0,
    "output_dir": "./deepOnet/runs/lab1data_v0.5.4",
    ...
    # Output files.
    "eval_csv_name": "thermodt_eval_metrics_v054.csv",
    "eval_summary_name": "thermodt_eval_summary_v054.json",
    "eval_sensor_summary_name": "thermodt_eval_sensor_summary_v054.csv",
    "eval_protocol_summary_name": "thermodt_eval_protocol_summary_v054.csv",
    "sensor_importance_summary_name": "thermodt_sensor_importance_v054.csv",
    "backward_elimination_path_name": "thermodt_backward_elimination_path_v054.csv",
    "backward_elimination_candidates_name": "thermodt_backward_elimination_candidates_v054.csv",
    "exhaustive_subset_search_name": "thermodt_exhaustive_subsets_v054.csv",
...
# -----------------------------------------------------------------------------
def compute_valid_window_end_positions(indices: np.ndarray, sequence_length: int) -> np.ndarray:
    indices = np.asarray(indices, dtype=int)
    if sequence_length <= 1:
        return np.sort(indices)
    index_set = set(int(i) for i in indices.tolist())
    valid = [
        int(end_idx)
        for end_idx in np.sort(indices)
        if (end_idx - sequence_length + 1) >= 0 and all((end_idx - offset) in index_set for offset in range(sequence_length))
    ]
...
class MultiSensorMaskedDataset(Dataset):
    def __init__(
        ...
        extra_mask_max: int,
        seed: int,
        branch_mode: str = "snapshot",
        sequence_length: int = 1,
    ):
        self.full_data_norm = np.asarray(full_data_norm, dtype=np.float32)
        self.endpoint_indices = np.asarray(endpoint_indices, dtype=int)
        ...
        self.n_time_total, self.n_sensors, self.n_channels = self.full_data_norm.shape
        if len(self.endpoint_indices) == 0:
            raise ValueError("No valid endpoint indices available for dataset.")

    def __len__(self) -> int:
        return len(self.endpoint_indices) * self.n_sensors

    def __getitem__(self, idx: int) -> Batch:
        t_local = idx // self.n_sensors
        s = idx % self.n_sensors
        end_idx = int(self.endpoint_indices[t_local])
        rng = np.random.default_rng(self.base_seed + idx)

        if self.branch_mode == "lstm":
            start_idx = end_idx - self.sequence_length + 1
            values = self.full_data_norm[start_idx:end_idx + 1].copy()   # (L, S, C)
            observed = np.ones((self.sequence_length, self.n_sensors), dtype=np.float32)
        else:
            values = self.full_data_norm[end_idx].copy()  # (S, C)
        ...
        if self.branch_mode == "lstm":
            ...
            branch_parts = [values.reshape(self.sequence_length, -1)]
        ...
        target = self.full_data_norm[end_idx, s, :].astype(np.float32)
        return Batch(
...
class LSTMBranchDeepONet(nn.Module):
    def __init__(
        self,
        branch_timestep_dim: int,
        coord_dim: int,
        output_dim: int,
        hidden: int,
        latent_dim: int,
        ...
        self.branch_lstm = nn.LSTM(
            input_size=int(branch_timestep_dim),
            hidden_size=int(lstm_hidden),
            num_layers=int(lstm_layers),
            dropout=float(lstm_dropout) if int(lstm_layers) > 1 else 0.0,
            batch_first=True,
        )
        self.branch_proj = nn.Linear(int(lstm_hidden), self.output_dim * self.latent_dim)
        self.trunk = MLP(coord_dim, hidden, self.output_dim * self.latent_dim, depth, activation)
        self.bias = nn.Parameter(torch.zeros(self.output_dim))

    def forward(self, branch_input: torch.Tensor, trunk_coord: torch.Tensor) -> torch.Tensor:
        if branch_input.dim() != 3:
            raise ValueError(f"LSTM branch expects input of shape (batch, seq, feat), got {tuple(branch_input.shape)}")
        _, (h_n, _) = self.branch_lstm(branch_input)
        branch = self.branch_proj(h_n[-1]).view(-1, self.output_dim, self.latent_dim)
        trunk = self.trunk(trunk_coord).view(-1, self.output_dim, self.latent_dim)
        return (branch * trunk).sum(dim=-1) + self.bias
...
def build_model(cfg: Mapping[str, Any], n_sensors: int, n_channels: int) -> nn.Module:
    branch_mode = str(cfg.get("branch_mode", "snapshot")).lower()
    if branch_mode == "lstm":
        timestep_dim = n_sensors * n_channels + (n_sensors if cfg["include_observed_mask"] else 0)
        return LSTMBranchDeepONet(
            branch_timestep_dim=timestep_dim,
            coord_dim=2,
            ...
            lstm_dropout=float(cfg.get("lstm_dropout", 0.0)),
        )
...
def build_dataloaders(cfg: Mapping[str, Any], loaded: LoadedData, device: torch.device) -> Tuple[DataLoader, DataLoader]:
    branch_mode = str(cfg.get("branch_mode", "snapshot")).lower()
    sequence_length = int(cfg.get("sequence_length", 1)) if branch_mode == "lstm" else 1
    train_endpoints = compute_valid_window_end_positions(loaded.splits.train_idx, sequence_length)
    val_endpoints = compute_valid_window_end_positions(loaded.splits.val_idx, sequence_length)
...
def train(cfg: MutableMapping[str, Any]) -> Tuple[Path, Path]:
    ...
    print("=" * 92)
    print(f"DeepONet v0.5.4 | experiment={cfg['experiment_name']} | device={device}")
    print("=" * 92)
    print(f"Variables         : {loaded.variable_names}")
    print(f"Sensors           : {len(loaded.sensor_ids)}")
    print(f"Timesteps         : {len(loaded.time_idx):,}")
    print(f"Branch mode       : {cfg.get('branch_mode', 'snapshot')}")
    if str(cfg.get('branch_mode', 'snapshot')).lower() == "lstm":
        print(f"Sequence length   : {int(cfg.get('sequence_length', 1))} timesteps")
    print(
        f"Train / Val / Test: {loaded.splits.split_meta['n_train']:,} / "
        f"{loaded.splits.split_meta['n_val']:,} / {loaded.splits.split_meta['n_test']:,}"
    )
    if str(cfg.get("branch_mode", "snapshot")).lower() == "lstm":
        train_endpoints = compute_valid_window_end_positions(loaded.splits.train_idx, int(cfg.get("sequence_length", 1)))
        val_endpoints = compute_valid_window_end_positions(loaded.splits.val_idx, int(cfg.get("sequence_length", 1)))
        test_endpoints = compute_valid_window_end_positions(loaded.splits.test_idx, int(cfg.get("sequence_length", 1)))
        print(f"Seq endpoints     : {len(train_endpoints):,} train / {len(val_endpoints):,} val / {len(test_endpoints):,} test")
...
# -----------------------------------------------------------------------------
@torch.no_grad()
def make_branch_input_from_window(
    ...
    branch_mode = str(cfg.get("branch_mode", "snapshot")).lower()
    sequence_length = int(cfg.get("sequence_length", 1)) if branch_mode == "lstm" else 1
    n_sensors = window_norm.shape[1]

    if branch_mode == "lstm":
        values = window_norm.copy()
        observed = np.ones((sequence_length, n_sensors), dtype=np.float32)
        values[:, removed_idx, :] = float(cfg["mask_fill_value"])
        observed[:, removed_idx] = 0.0
        branch_parts = [values.reshape(sequence_length, -1)]
        if cfg["include_observed_mask"]:
            branch_parts.append(observed)
        return np.concatenate(branch_parts, axis=1).astype(np.float32)

    values = window_norm[-1].copy()
    ...
    if cfg["include_observed_mask"]:
        branch_parts.append(observed)
    return np.concatenate(branch_parts, axis=0).astype(np.float32)
...
def predict_protocol(
    ...
    branch_mode = str(cfg.get("branch_mode", "snapshot")).lower()
    sequence_length = int(cfg.get("sequence_length", 1)) if branch_mode == "lstm" else 1
    n_time = len(endpoint_indices)
    n_sensors = len(sensor_ids)
    n_variables = full_data_norm.shape[-1]
    ...
    for repeat in range(protocol.repeat_count):
        ...
        for s_idx, sensor_id in enumerate(sensor_ids):
            ...
            for end_idx in endpoint_indices:
                ...
                if branch_mode == "lstm":
                    start_idx = int(end_idx) - sequence_length + 1
                    window_norm = full_data_norm[start_idx:int(end_idx) + 1]
                else:
                    window_norm = full_data_norm[int(end_idx):int(end_idx) + 1]
                branch_batch.append(make_branch_input_from_window(window_norm, removed_idx, cfg))
            branch_batch_np = np.stack(branch_batch, axis=0).astype(np.float32)
            coord_batch = np.repeat(coords_norm[s_idx:s_idx + 1], n_time, axis=0).astype(np.float32)
            pred = model(
                torch.from_numpy(branch_batch_np).to(device),
...
def evaluate_single_protocol(
    ...
    branch_mode = str(cfg.get("branch_mode", "snapshot")).lower()
    sequence_length = int(cfg.get("sequence_length", 1)) if branch_mode == "lstm" else 1
    test_endpoints = compute_valid_window_end_positions(test_idx, sequence_length)
    test_data_norm = loaded.data_norm[test_endpoints]
    ...
    pred_repeats = predict_protocol(model, protocol, cfg, loaded.data_norm, test_endpoints, coords_norm, sensor_ids, device)
...
def save_timeseries_plots(outputs: EvalOutputs, protocol_name: str, display_name: str, masked_total: int) -> None:
    ...
    for c_idx, variable in enumerate(outputs.variable_names):
        ...
        fig.suptitle(
            f"DeepONet v0.5.4 — {variable.capitalize()} | {display_name} | masked_total={masked_total}",
            fontsize=12,
            fontweight="bold",
        )
        plt.tight_layout()
...
def evaluate(cfg: MutableMapping[str, Any], checkpoint_path: Path) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    outputs = EvalOutputs(
        ...
        time_test=loaded.time_idx[test_endpoints],
        sensor_ids=[int(x) for x in checkpoint["sensors"]["sensor_ids"]],
        variable_names=list(checkpoint["variables"]),
