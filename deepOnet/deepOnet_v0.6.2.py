...
"""
DeepONet v0.6.2 — Step 6 Phase 2 (Lab1 -> Lab2 transfer with subset combinations)
=====================================================================================

Built from the v0.5.3 deep MLP DeepONet, with:
- blocked 80/10/10 train/val/test split for Lab1
- blocked Lab2 blocked split:
    * 10% held-out test
    * 16% adaptation train
    * 4% adaptation val
    * remaining rows unused
- Phase 2 transfer modes:
    * head-only adaptation
    * partial fine-tuning
- optional zero-shot / full fine-tuning retained behind booleans
- Lab2 evaluation over combinations of active-sensor subsets with k in {5, 3, 2}
  using exhaustive or sampled protocol generation per-k

Notes
-----
- By default, Phase 2 focuses on head-only and partial fine-tuning.
- Exhaustive evaluation for k=5 may be expensive. The default config samples a
  manageable number of 5-sensor combinations, while evaluating all 3-sensor and
...
PARAMS: Dict[str, Any] = {
    "mode": "phase2_transfer",  # train_lab1, phase2_transfer, train_and_phase2
    "seed": 42,
    "device": "auto",  # auto, cpu, cuda, mps
    "experiment_name": "lab1_to_lab2_transfer_v0.6.2",
    "data_root": "./data",
    "resolution": "5min",
    ...
    # Phase-2 transfer modes.
    "run_lab1_test_eval": False,
    "run_lab2_zero_shot": False,
    "run_lab2_head_only": True,
    "run_lab2_partial_finetune": True,
    "run_lab2_full_finetune": False,
    ...
    # Phase-2 Lab2 protocol generation.
    "lab2_phase2_active_counts": [5, 3, 2],
    # Per-k generation mode: "sampled" or "exhaustive".
    "lab2_phase2_generation_mode": {
        5: "sampled",
        3: "exhaustive",
        2: "exhaustive",
    },
    # For sampled modes, cap the number of subsets per k. None means all.
    "lab2_phase2_sample_limit": {
        5: 20,
        3: 150,
        2: None,
    },
    "lab2_phase2_sample_seed_offset": 700,

    # Output.
    "output_dir": "./deepOnet/runs/lab1_to_lab2_v0.6.2",
    ...
    "lab1_eval_metrics_name": "thermodt_lab1_test_metrics_v062.csv",
    "phase2_metrics_name": "thermodt_phase2_lab2_metrics_v062.csv",
    "phase2_sensor_summary_name": "thermodt_phase2_lab2_sensor_summary_v062.csv",
    "phase2_protocol_summary_name": "thermodt_phase2_lab2_protocol_summary_v062.csv",
    "phase2_best_subsets_name": "thermodt_phase2_lab2_best_subsets_v062.csv",
    "phase2_summary_name": "thermodt_phase2_lab2_summary_v062.json",
}
...
def train_model(
    ...
    tag: str = "train",
    transfer_train_mode: str | None = None,
        ...
        if is_improved(val_loss, best_val, min_delta):
            ...
            torch.save(
                package_checkpoint_payload(
                    ...
                    extra={
                        "tag": tag,
                        "transfer_train_mode": transfer_train_mode or "full",
...
def build_phase2_protocols(cfg: Mapping[str, Any], sensor_ids: Sequence[int]) -> List[TransferProtocol]:
    sensor_ids = [int(s) for s in sensor_ids]
    sensor_set = set(sensor_ids)
    rng = np.random.default_rng(int(cfg["seed"]) + int(cfg.get("lab2_phase2_sample_seed_offset", 700)))
    protocols: List[TransferProtocol] = []

    for k in [int(x) for x in cfg["lab2_phase2_active_counts"]]:
        if k <= 0 or k > len(sensor_ids):
            raise ValueError(f"Invalid active-count k={k} for {len(sensor_ids)} sensors")
        combos = [tuple(int(x) for x in c) for c in itertools.combinations(sensor_ids, k)]
        mode_map = cfg.get("lab2_phase2_generation_mode", {})
        limit_map = cfg.get("lab2_phase2_sample_limit", {})
        mode = str(mode_map.get(k, "exhaustive")).lower()
        limit = limit_map.get(k, None)
        ...
        for active in combos:
            ...
            if bad:
                raise ValueError(f"Active set references unknown Lab2 sensor ids: {bad}")
            protocols.append(
                TransferProtocol(
                    protocol_name=_protocol_name_from_active(active),
                    display_name=f"Lab2 active set {list(active)}",
                    active_sensors=list(active),
                )
            )
    return protocols
...
def evaluate_active_protocols(
    ...
    protocols: Sequence[TransferProtocol],
    device: torch.device,
    transfer_mode: str,
...
def main(cfg: Dict[str, Any] | None = None) -> None:
    ...
    if mode in ("train_lab1", "train_and_phase2"):
        train_model(
            cfg=cfg,
    ...
    if bool(cfg.get("run_lab1_test_eval", False)):
        ref_protocols = build_phase2_protocols(cfg, lab1_norm.sensor_ids)
    ...
    phase2_protocols = build_phase2_protocols(cfg, lab2_norm.sensor_ids)
    summary_json["phase2_protocol_count"] = int(len(phase2_protocols))

    all_metrics: List[pd.DataFrame] = []
    all_sensor_summary: List[pd.DataFrame] = []
    all_protocol_summary: List[pd.DataFrame] = []

    def _append_results(mode_name: str, metrics: pd.DataFrame, sensor_summary: pd.DataFrame, protocol_summary: pd.DataFrame, extra: Mapping[str, Any] | None = None):
        all_metrics.append(metrics)
        all_sensor_summary.append(sensor_summary)
        all_protocol_summary.append(protocol_summary)
        payload: Dict[str, Any] = {"results": protocol_summary.to_dict(orient="records")}
        if extra:
            payload.update(copy.deepcopy(dict(extra)))
        summary_json["phase2_lab2"][mode_name] = payload
        print("=" * 92)
        print(f"Lab2 {mode_name} evaluation complete")
        print("=" * 92)
        print(protocol_summary.set_index(["protocol_name", "masked_total", "variable"])[["rmse", "mae", "r2"]])

    # ---- Optional zero-shot ----
    if bool(cfg.get("run_lab2_zero_shot", False)):
        zero_metrics, zero_sensor_summary, zero_protocol_summary = evaluate_active_protocols(
            lab1_model, lab2_norm, lab2_split.masks["test"], phase2_protocols, device, transfer_mode="zero_shot"
        )
        _append_results("zero_shot", zero_metrics, zero_sensor_summary, zero_protocol_summary)

    # ---- Head-only ----
    if bool(cfg.get("run_lab2_head_only", True)):
        head_best, head_final, head_meta = train_model(
            cfg=cfg,
            ...
            device=device,
            out_dir=out_dir,
            best_name=str(cfg["head_best_checkpoint_name"]),
            final_name=str(cfg["head_final_checkpoint_name"]),
            lr=float(cfg["head_only_lr"]),
            epochs=int(cfg["head_only_epochs"]),
            patience=int(cfg["head_only_patience"]),
            min_delta=float(cfg["head_only_min_delta"]),
            seed=int(cfg["seed"]) + 5100,
            init_state_dict=lab1_ckpt["model_state"],
            tag="lab2_head_only",
            transfer_train_mode="head_only",
        )
        head_ckpt = torch.load(head_best, map_location=device)
        head_model = build_model(cfg, len(lab2_norm.sensor_ids), len(lab2_norm.variable_names)).to(device)
        head_model.load_state_dict(head_ckpt["model_state"])
        head_metrics, head_sensor_summary, head_protocol_summary = evaluate_active_protocols(
            head_model, lab2_norm, lab2_split.masks["test"], phase2_protocols, device, transfer_mode="head_only"
        )
        _append_results("head_only", head_metrics, head_sensor_summary, head_protocol_summary, extra={"training_meta": head_meta})

    # ---- Partial fine-tune ----
    if bool(cfg.get("run_lab2_partial_finetune", True)):
        partial_best, partial_final, partial_meta = train_model(
            cfg=cfg,
            normalized=lab2_norm,
            split_masks=lab2_split,
            train_key="adapt_train",
            ...
            device=device,
            out_dir=out_dir,
            best_name=str(cfg["partial_best_checkpoint_name"]),
            final_name=str(cfg["partial_final_checkpoint_name"]),
            lr=float(cfg["partial_finetune_lr"]),
            epochs=int(cfg["partial_finetune_epochs"]),
            patience=int(cfg["partial_finetune_patience"]),
            min_delta=float(cfg["partial_finetune_min_delta"]),
            seed=int(cfg["seed"]) + 5200,
            init_state_dict=lab1_ckpt["model_state"],
            tag="lab2_partial_finetune",
            transfer_train_mode="partial",
        )
        partial_ckpt = torch.load(partial_best, map_location=device)
        partial_model = build_model(cfg, len(lab2_norm.sensor_ids), len(lab2_norm.variable_names)).to(device)
        partial_model.load_state_dict(partial_ckpt["model_state"])
        partial_metrics, partial_sensor_summary, partial_protocol_summary = evaluate_active_protocols(
            partial_model, lab2_norm, lab2_split.masks["test"], phase2_protocols, device, transfer_mode="partial_finetune"
        )
        _append_results("partial_finetune", partial_metrics, partial_sensor_summary, partial_protocol_summary, extra={"training_meta": partial_meta})

    # ---- Optional full fine-tune ----
    if bool(cfg.get("run_lab2_full_finetune", False)):
        ft_best, ft_final, ft_meta = train_model(
            ...
            seed=int(cfg["seed"]) + 5300,
            init_state_dict=lab1_ckpt["model_state"],
            tag="lab2_full_finetune",
            transfer_train_mode="full",
        )
        ...
        ft_metrics, ft_sensor_summary, ft_protocol_summary = evaluate_active_protocols(
            ft_model, lab2_norm, lab2_split.masks["test"], phase2_protocols, device, transfer_mode="full_finetune"
        )
        _append_results("full_finetune", ft_metrics, ft_sensor_summary, ft_protocol_summary, extra={"training_meta": ft_meta})
    ...
    if all_metrics:
        ...
        # Composite ranking for best subsets by transfer mode and active-count.
        best_subsets_df = (
            protocol_summary_df
            .pivot_table(index=["transfer_mode", "protocol_name", "display_name", "masked_total"], columns="variable", values="r2")
            .reset_index()
        )
        for col in ["temperature", "humidity"]:
            if col not in best_subsets_df.columns:
                best_subsets_df[col] = np.nan
        best_subsets_df["active_count"] = len(lab2_norm.sensor_ids) - best_subsets_df["masked_total"]
        best_subsets_df["score"] = best_subsets_df[["temperature", "humidity"]].mean(axis=1)
        best_subsets_df = best_subsets_df.rename(columns={"temperature": "r2_temperature", "humidity": "r2_humidity"})
        best_subsets_df = best_subsets_df.sort_values(["transfer_mode", "active_count", "score"], ascending=[True, True, False]).reset_index(drop=True)

        metrics_path = out_dir / str(cfg["phase2_metrics_name"])
        sensor_summary_path = out_dir / str(cfg["phase2_sensor_summary_name"])
        protocol_summary_path = out_dir / str(cfg["phase2_protocol_summary_name"])
        best_subsets_path = out_dir / str(cfg["phase2_best_subsets_name"])
        summary_path = out_dir / str(cfg["phase2_summary_name"])
