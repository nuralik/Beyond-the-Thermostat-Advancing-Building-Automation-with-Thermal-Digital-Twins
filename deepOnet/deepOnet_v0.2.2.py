...
"""
DeepONet v0.2.2 — multi-output baseline with heavier masking + early stopping
================================================================

Built from the v0.2 workflow, with the following upgrades:
- multi-output reconstruction for temperature and humidity
- heavier multi-sensor masking by default for step 3
- early stopping based on validation non-improvement patience
...
PARAMS: Dict[str, Any] = {
    ...
    "experiment_name": "lab1_temp_humidity_v0.2.2",
    "data_root": "./data",
    "lab": "lab1",
    "resolution": "5min",
    ...
    "train_extra_mask_max": 5,
    "eval_masked_total": [1, 3, 6, 9, 12],
    "eval_repeats": 3,
    "hidden": 128,
    "latent_dim": 128,
    "depth": 4,
    ...
    "weight_decay": 0.0,
    "num_workers": 0,
    "output_dir": "./deepOnet/runs/lab1data_v0.2.2",
...
def train(cfg: MutableMapping[str, Any]) -> Tuple[Path, Path]:
    ...
    print("=" * 76)
    print(f"DeepONet v0.2.2 | experiment={cfg['experiment_name']} | device={device}")
    print("=" * 76)
...
def save_timeseries_plots(outputs: EvalOutputs, masked_total: int) -> None:
    ...
    for c_idx, variable in enumerate(outputs.variable_names):
        ...
        fig.suptitle(
            f"DeepONet v0.2.2 — {variable.capitalize()} test-set reconstruction | masked_total={masked_total}",
            fontsize=12,
            fontweight="bold",
        )
        plt.tight_layout()
...
def save_scatter_plots(outputs: EvalOutputs, masked_total: int, point_size: float) -> None:
    ...
    for c_idx, variable in enumerate(outputs.variable_names):
        ...
        fig.suptitle(
            f"DeepONet v0.2.2 — {variable.capitalize()} predicted vs actual | masked_total={masked_total}",
            fontsize=12,
            fontweight="bold",
        )
        plt.tight_layout()
...
def save_floormap_plots(outputs: EvalOutputs, masked_total: int, grid_res: int) -> None:
    ...
    for variable in outputs.variable_names:
        ...
        fig.suptitle(
            f"DeepONet v0.2.2 — {variable.capitalize()} floor-map metrics | masked_total={masked_total}",
            fontsize=12,
            fontweight="bold",
        )
